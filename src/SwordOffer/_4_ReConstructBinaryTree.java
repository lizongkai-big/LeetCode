...
public class _4_ReConstructBinaryTree {
    ...
    public TreeNode reConstructBinaryTreeIterative(int [] pre, int [] in) {
        if(pre == null || pre.length == 0)
            return null;
        int j = 0;
        while(in[j] != pre[0]) {j ++;}
        TreeNode node = new TreeNode(pre[0]);
        node.left = reConstructBinaryTreeIterative(Arrays.copyOfRange(pre, 1, j+1), Arrays.copyOfRange(in, 0, j));
        node.right = reConstructBinaryTreeIterative(Arrays.copyOfRange(pre, j+1, pre.length), Arrays.copyOfRange(in, j+1, in.length));
        return node;
    }
}
